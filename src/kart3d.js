...
(function () {
  ...
  function push(list, v, c, sm, vn) { list.push({ v: v, c: c, sm: !!sm, vn: vn }); }
  ...
  function cylX(list, cx, cy, cz, r, hw, segs, side, cap, sm) {
    ...
    for (i = 0; i < segs; i++) {
      a = (i / segs) * Math.PI * 2;
      N.push([0, Math.cos(a), Math.sin(a)]);
    }
    for (i = 0; i < segs; i++) {
      j = (i + 1) % segs;
      // per-vertex normals turn 16 flat bands into one continuous curve
      push(list, [L[i], L[j], R[j], R[i]], side, sm, [N[i], N[j], N[j], N[i]]);
    }
    ...
    for (i = 0; i < lat; i++) {
      ...
      for (j = 0; j < lon; j++) {
        ...
        push(list, [P(t0,f0), P(t0,f1), P(t1,f1), P(t1,f0)],
             ...
             [NV(t0,f0), NV(t0,f1), NV(t1,f1), NV(t1,f0)]);
      }
    }
  }
  ...
  function lum(n, ao) {
    ...
    return { k: (0.58 + 0.50 * kd + 0.16 * fd) * ao, rim: 0.26 * rd * rd * ao };
  }

  function tint(c, t) {
    return "rgb(" + cl(c[0]*t.k + t.rim*116) + "," +
                    cl(c[1]*t.k + t.rim*144) + "," +
                    cl(c[2]*t.k + t.rim*182) + ")";
  }

  function shade(c, n, ao) { return tint(c, lum(n, ao)); }

  /* ─────────────────────────────── render ──────────────────────────────── */
  var drive = 0, spin = 0, W = 0, H = 0, dpr = 1;
  // Backing-store scale. Sampling above CSS resolution is what smooths the long
  // diagonals; canvas's own edge AA is not enough at 1x. Measured on this model at
  // a 720x424 canvas: 1x 6.7ms, 1.5x 9.4ms, 2x 11.4ms per frame. The loop drops
  // this to 1 if the device turns out not to afford it.
  var maxScale = 2;

  function resize() {
    var r = canvas.getBoundingClientRect();
    if (!r.width) return false;
    dpr = Math.min(Math.max(window.devicePixelRatio || 1, 1.5), maxScale);
  ...
  function frame() {
    ...
    for (k = 0; k < wheels.length; k++) {
      ...
      for (j = 0; j < wl.f.length; j++) {
        var src = wl.f[j].v, dst = [], srcN = wl.f[j].vn, dstN = srcN ? [] : null;
        for (i = 0; i < src.length; i++) {
          p = rotX(src[i], spin);
          if (wl.steer) p = rotY(p, steer);
          dst.push([p[0] + wl.c[0], p[1] + wl.c[1], p[2] + wl.c[2]]);
          if (srcN) {
            var q = rotX(srcN[i], spin);
            dstN.push(wl.steer ? rotY(q, steer) : q);
          }
        }
        list.push({ v: dst, c: wl.f[j].c, sm: wl.f[j].sm, vn: dstN });
      }
    }
    ...
    for (i = 0; i < list.length; i++) {
      ...
      cxw /= n; cyw /= n; czw /= n;
      var flip = nx * (0 - cxw) + ny * (camY - cyw) + nz * (dist - czw) < 0;
      ...
      // Curved surfaces carry per-vertex normals. Canvas cannot interpolate a
      // colour across a polygon, but shading on a tyre or a helmet varies mostly
      // along one direction, so a two-stop linear gradient between the darkest
      // and the brightest vertex reads as smooth and costs one gradient object.
      var fill, edge, vn = list[i].vn;
      if (vn) {
        var lo = 1e9, hi = -1e9, iLo = 0, iHi = 0, tints = [], vnj, tj, bj;
        for (j = 0; j < n; j++) {
          vnj = rotX(rotY(vn[j], yaw), lean);
          if (flip) { vnj = [-vnj[0], -vnj[1], -vnj[2]]; }
          tj = lum(vnj, ao);
          tints.push(tj);
          bj = tj.k + tj.rim;
          if (bj < lo) { lo = bj; iLo = j; }
          if (bj > hi) { hi = bj; iHi = j; }
        }
        edge = tint(list[i].c, tints[iHi]);
        if (hi - lo < 0.012 || iLo === iHi) {
          fill = edge;
        } else {
          var gr = ctx.createLinearGradient(pts[iLo][0], pts[iLo][1], pts[iHi][0], pts[iHi][1]);
          gr.addColorStop(0, tint(list[i].c, tints[iLo]));
          gr.addColorStop(1, edge);
          fill = gr;
        }
      } else {
        fill = edge = shade(list[i].c, [nx, ny, nz], ao);
      }

      out.push({ p: pts, z: zsum / n, s: fill, sm: list[i].sm, n: nz, edge: edge });
    }
    ...
    for (i = 0; i < out.length; i++) {
      ...
      // a gradient is not a valid stroke colour here — smooth faces stroke with
      // their own brightest tint to close the seam
      if (o.sm && o.n > 0.34) { ctx.strokeStyle = o.edge; ctx.lineWidth = 1; }
  ...
  function loop(ts) {
    ...
    var ms = last ? ts - last : 16.67;
    var dt = Math.min(ms / 16.67, 3);
    last = ts;

    // Rather than guess at device capability, watch the actual frame time and
    // give up the supersampling once if this machine cannot hold the budget.
    // One-way, so it cannot oscillate between the two costs.
    if (maxScale > 1) {
      if (ms > 26) overBudget++; else if (overBudget) overBudget--;
      if (overBudget > 30) { maxScale = 1; overBudget = 0; resize(); }
    }
